...
import {
  projectsOverviewFetchAction,
  projectsOverviewFetchByIndustryAction,
} from "../../store/actions/projectsOverviewActions";
  ...
  return (
    <>
      ...
      <ButtonGroup color="secondary" aria-label="outlined primary button group">
        <Button onClick={() => handleAllClicked()}>Vis alle</Button>

        {industries.map((industry) => (
          <Button
            key={industry.id}
            onClick={() => handleIndustryClicked(industry.name)}
          >
            {industry.name}
          </Button>
        ))}
